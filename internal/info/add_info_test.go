...
Another test file`
	...
	} else if annotation.Description != "A test file" {
		t.Errorf("Wrong description. Expected: 'A test file', Got: '%s'", annotation.Description)
	}
}
...
A test file`
	...
	if annotation != nil {
		t.Error("Annotation should be nil for non-existing entry")
	}
}
...
func TestEntryExists_NoInfoFile(t *testing.T) {
	...
	// Test entry in directory without .info file
	exists, annotation, err := EntryExists(tempDir, "test.txt")
	if err != nil {
		t.Fatalf("EntryExists should not fail when .info file doesn't exist: %v", err)
	}
...
func TestAddOrUpdateEntry_ComplexWorkflow(t *testing.T) {
	...
	// Add second entry
	err = AddOrUpdateEntry(tempDir, "main.go", "Application entry point", UpdateActionReplace)
	if err != nil {
		t.Fatalf("Failed to add second entry: %v", err)
	}
	
	// Update first entry with append
	err = AddOrUpdateEntry(tempDir, "README.md", "Updated with examples", UpdateActionAppend)
	if err != nil {
		t.Fatalf("Failed to append to first entry: %v", err)
	}
	
	// Update second entry with replace
	err = AddOrUpdateEntry(tempDir, "main.go", "Main application file", UpdateActionReplace)
	if err != nil {
		t.Fatalf("Failed to replace second entry: %v", err)
	}
	...
	// Check README.md (should be appended)
	...
	// Check main.go (should be replaced)
	mainGo, exists := annotations["main.go"]
	if !exists {
		t.Error("main.go annotation not found")
	} else if mainGo.Description != "Main application file" {
		t.Errorf("main.go description mismatch.\nExpected: 'Main application file'\nGot: '%s'", mainGo.Description)
	}
} 